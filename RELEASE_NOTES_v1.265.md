...
| File | Change |
|---|---|
| `index.html` | RO Description bug fixes (4 issues) |
...
- First fix attempt used `prompt:''` (silent refresh) which fails when the token is fully expired — replaced with `prompt:'consent'` to force the interactive Google OAuth popup
